...
import search_algorithm as search
...
def makeandplot(map,ss,info=[]):
    plt.clf()
    start=[np.where(map==-2)[1][0],np.where(map==-2)[0][0]]
    goal=[np.where(map==-3)[1][0],np.where(map==-3)[0][0]]
    plt.imshow(map)
    plt.show()
    path=[]
    if ss=="BFS":
        path,been_here=search.search(map,start,goal)
    elif ss=="DFS":
        path,been_here=search.searchdepth(map,start,goal)
    elif ss=="greedy":
        path,been_here=search.greedy(map,start,goal)
    elif ss=="A*M":
        path,been_here=search.astarManhattan(map,start,goal)
    elif ss=="A*E":
        path,been_here=search.astarEuclidean(map,start,goal)
    elif ss=="srand":
        path,been_here=search.random_search(map,start,goal)
    elif ss=="A*new":
        path,been_here=search.astarnew(map,start,goal,info)
    else:
        been_here=[]
        print("nothing")
    #print("example: ", path)
...
#makeandplot(_map_,"BFS")
#_map_ = generateMap2d([60, 60])
#makeandplot(_map_,"DFS")
#_map_ = generateMap2d([60, 60])
#makeandplot(_map_,"greedy")
#_map_ = generateMap2d([60, 60])
#makeandplot(_map_,"A*M")
#_map_ = generateMap2d([60, 60])
#makeandplot(_map_,"A*E")
_map_ = generateMap2d([60, 60])
makeandplot(_map_,"srand")
#_map_,info = generateMap2d_obstacle([60,60])
#makeandplot(_map_,"DFS")
#_map_,info = generateMap2d_obstacle([60,60])
#makeandplot(_map_,"BFS")
#_map_,info = generateMap2d_obstacle([60,60])
#print(makeandplot(_map_,"A*M"))
#_map_,info = generateMap2d_obstacle([60,60])
...
for i in range(1):
    ...
    bfs=mkpath(_map_,"BFS")
    greedy=mkpath(_map_,"greedy")
